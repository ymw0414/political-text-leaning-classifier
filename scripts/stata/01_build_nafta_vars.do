...
* =========================================================================== *
* [SELF-RUN MODE] Standalone Execution Support                                *
* =========================================================================== *
if "$root" == "" {
    clear all
    set more off
...
display ">>> [Step 01] Building Variables... This may take a while."

***********************************************************
* Section 1: Ad Valorem Equivalent Tariff                 *
***********************************************************
display ">>> Processing Section 1: Tariffs..."
...
foreach y in 90 91 92 93 94 95 96 97 98 99 00 01 {
    ...
    save "$processed/tariff_romalis_`y'.dta", replace
}

clear
foreach y in 90 91 92 93 94 95 96 97 98 99 00 01 {
    append using "$processed/tariff_romalis_`y'.dta"
}
replace year = year + 1900 if year < 100 
save "$processed/tariff_romalis_all", replace 
...
forvalues y = 2002/2008 {
    ...
    save "$processed/tariff_usitc_`y'.dta", replace 
}

clear
forvalues y = 2002/2008 {
    append using "$processed/tariff_usitc_`y'.dta"
}
save "$processed/tariff_usitc_all", replace

* ---------- Merging unitvalue ---------- *
import excel "$raw_data/import/import_usitc/dataweb-queryExport.xlsx", ///
    sheet("Customs Value") cellrange(A3:W11599) firstrow clear
    
reshape long Year, i(HTSNumber QuantityDescription) j(year)
rename Year value
save "$processed/value", replace

import excel "$raw_data/import/import_usitc/dataweb-queryExport.xlsx", ///
    sheet("First Unit of Quantity") cellrange(A3:W11599) firstrow clear
    
reshape long Year, i(HTSNumber QuantityDescription) j(year)
rename Year quantity
merge 1:1 HTSNumber QuantityDescription year using "$processed/value"

keep if year >= 2002
rename HTSNumber hts8
gen unitvalue = value/quantity 
...
destring hts8, replace  
save "$processed/unitvalue_1990-2008.dta", replace

use "$processed/tariff_usitc_all", clear
merge m:1 hts8 year quantity_1_code using "$processed/unitvalue_1990-2008.dta"
keep if _merge == 1 | _merge == 3
drop _merge
destring mexico_rate_type_code, replace force
save "$processed/tariff_usitc_all_unitvalue", replace

* ---------- Calculating ad valorem equivalent tariff rate ---------- *
use "$processed/tariff_romalis_all", clear
append using "$processed/tariff_usitc_all_unitvalue"
...
bysort year hts8: egen tariff_AVE_max = max(tariff_AVE) 
drop tariff_AVE
duplicates drop (year hts8), force
rename tariff_AVE_max tariff_AVE
save "$processed/tariff_all", replace


***********************************************************
* Section 2: Weights and RCA                              *
***********************************************************
display ">>> Processing Section 2: Weights and RCA..."

* ---------- Imports ---------- *
use "$raw_data/import/import_hakobyan_mclaren/imports1990-2000", clear
keep if year == 1990
drop year
save "$processed/import_1990", replace

use "$processed/tariff_all", clear
merge m:1 hts8 using "$processed/import_1990"
keep if _merge == 3
drop _merge
gen hts6 = floor(hts8 / 100)
rename hts6 hs6 
save "$processed/tariff_all_import", replace

* ---------- Exports ---------- *
import delimited "$raw_data/export/export_hakobyan_mclaren/worldex19902000.csv", clear
keep if year == 1990
...
save "$processed/export_1990", replace

use "$processed/tariff_all_import", clear
merge m:1 hs6 using "$processed/export_1990"
keep if _merge == 3
drop _merge
save "$processed/tariff_all_import_export", replace
...
replace share = 1 if duplicates == 1 
drop weights_method duplicates
save "$processed/hs6_sic", replace

use "$processed/tariff_all_import_export", clear
joinby hs6 using "$processed/hs6_sic"
...
gen weight_im = mex_im / tot_mex_im 
...
drop temp*
save "$processed/tariff_rca_MEX", replace


***********************************************************
* Section 3: Employment                                   *
***********************************************************
display ">>> Processing Section 3: Employment..."
...
foreach year in 86 87 88 89 90 91 92 93 94 95 96 97 1998 1999 2000 2001 2002 2003 2004 2005 2006 2007 2008 {
    ...
    save "$processed/cbp`year'co.dta", replace
}

clear
foreach year in 86 87 88 89 90 91 92 93 94 95 96 97 1998 1999 2000 2001 2002 2003 2004 2005 2006 2007 2008 {
    append using "$processed/cbp`year'co.dta"
}
drop if fipscty == 999
gen county = fipstate * 1000 + fipscty

gen emp_tot = emp if sic == "----" | naics == "------" 
...
save "$processed/employment", replace


***********************************************************
* Section 4: Vulnerability Measure                        *
***********************************************************
display ">>> Processing Section 4: Vulnerability Measure..."

use "$processed/employment", clear 
keep if year == 1990
keep fipstate county sic emp
rename emp emp1990
joinby sic using "$processed/tariff_rca_MEX", unmatched(master) 
...
keep fipstate county year vulnerability* rename fipstate state
drop if inlist(state, 2,15) 

preserve
    keep if year == 1990
    xtile quartile = vulnerability1990, n(4)
    keep county quartile
    save "$processed/quartile", replace
restore

merge m:1 county using "$processed/quartile"
drop _merge
...
preserve
    ...
    save "$processed/vulnerability_scaled", replace
restore

merge m:1 county using "$processed/vulnerability_scaled"
drop _merge
save "$processed/vulnerability", replace


***********************************************************
* Section 5: Pop./Edu./Manu. Share/Income                 *
***********************************************************
display ">>> Processing Section 5: Demographics..."
...
forvalues year = 1986/1989 {
    ...
    collapse (sum) pop_*, by(state county year) 
    
    keep year county state pop_total pop_working 
    save "$processed/pop`year'.dta", replace
}

clear 
forvalues year = 1986/1989 {
    append using "$processed/pop`year'.dta"
}
save "$processed/pop_80s", replace

* ---------- Population: 1990-1999 ---------- *
forvalues year = 1990/1999 {
    use "$raw_data/demographic/stch`year'.dta", clear
    bysort state county: egen pop_total = total(pop) 
    bysort state county: egen pop_working = total(pop) if inrange(agegroup, 4, 13)
...
save "$processed/pop_90s", replace

* ---------- Population: 2000-2008 ---------- *
use "$raw_data/demographic/coest00intalldata.dta", clear
drop if inlist(yearref, 1, 12) 
...
save "$processed/pop_00s", replace

clear 
foreach file in pop_80s pop_90s pop_00s {
    append using "$processed/`file'.dta"
} 
...
drop temp
save "$processed/pop_all", replace 
...
keep (county less_highschool1990 highschool1990 college1990 bachelor_higher1990) 
drop if substr(county, -3, 3) == "000" 
gen state = substr(county, 1, 2)
destring state county, replace
...
save "$processed/education1990", replace
...
preserve
    ...
    replace county_name = lower(ustrtrim(county_name)) 
    save "$processed/countyfip_name_crosswalk_lower", replace
restore

merge 1:1 county_name st_abrv using "$processed/countyfip_name_crosswalk_lower" 
keep if _merge == 3
drop _merge

rename (statefip countyfips county_income_*) (state county income*)
keep state county income1989
save "$processed/income", replace

* ---------- Merging population, education, manufacturing share, income ---------- *
use "$processed/vulnerability", clear

merge 1:1 year county using "$processed/pop_all"
keep if inlist(_merge, 1, 3)
drop _merge

merge m:1 county using "$processed/education1990"
keep if inlist(_merge, 1, 3)
drop _merge

merge m:1 county using "$processed/manushare1990"
keep if inlist(_merge, 1, 3)
drop _merge

merge m:1 county using "$processed/income"
keep if inlist(_merge, 1, 3)
drop _merge

save "$processed/data_section5", replace
...
save "$processed/employment_county", replace

use "$processed/data_section5", clear

merge 1:1 year county using "$processed/employment_county"
drop _merge
...
* Fill missing values within county using max value (time-invariant variables)
foreach var of varlist vulnerability1990_scaled manushare1990 pop1990_total {
...
* This is the final economic dataset (County-Year Panel)
* Renamed to be more descriptive (was data_figure2.dta)
save "$processed/county_econ_panel", replace

display ">>> [Step 01] Complete! File saved: $processed/county_econ_panel.dta"